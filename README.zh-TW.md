...
管理來源、更新與將此範圍標為已讀改為「訂閱來源」標題右側的圖示按鈕，滑鼠停留可查看名稱。閱讀器隱藏額外的 Obsidian 檢視標題列，保留更多內容空間。全文上方提供 **Back to list**、**Previous**、**Next**；`Esc` 返回列表並保留選取位置。搜尋、篩選和 **Reading actions** 批次操作留在列表畫面。
