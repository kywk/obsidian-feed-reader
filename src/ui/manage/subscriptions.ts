...
export class ManageSubscriptionsView extends ItemView {
  ...
  private heading(title: string, page: typeof this.page): void {
    ...
    if (page !== 'list') new Setting(this.contentEl).addButton(button => button.setButtonText('Back').onClick(() => this.render()));
  }
  private render(): void {
    this.heading('Manage RSS sources', 'list');
    ...
    for (const section of ['sources', 'folders'] as const) {
      const tab = tabs.createEl('button', { text: section === 'sources' ? `Sources (${snapshot.document.feeds.length})` : `Folders (${snapshot.document.folders.length})`, attr: { 'aria-pressed': String(this.section === section) } });
      tab.addEventListener('click', () => { this.section = section; this.render(); });
    }
    if (this.section === 'sources') {
      const search = this.contentEl.createEl('input', { type: 'search', cls: 'vfr-manager-search', placeholder: 'Search sources, URLs or folders', attr: { 'aria-label': 'Search sources' } });
      ...
      const renderResults = (): void => {
        ...
        controls.createSpan({ text: feeds.length ? `${this.offset + 1}–${Math.min(this.offset + 50, feeds.length)} of ${feeds.length} sources` : 'No matching sources' });
        const prev = controls.createEl('button', { text: 'Previous page' }); prev.disabled = this.offset === 0;
        const next = controls.createEl('button', { text: 'Next page' }); next.disabled = this.offset + 50 >= feeds.length;
        ...
        for (const feed of feeds.slice(this.offset, this.offset + 50)) {
          ...
          new Setting(row).setName(feed.title).setDesc(`${feed.url} · ${folders.join(', ') || 'Unfiled'}`)
            .addButton(button => button.setButtonText('Edit / folders').setDisabled(locked).onClick(() => this.editFeed(feed)))
            .addButton(button => button.setButtonText('Refresh').onClick(() => { void this.run(() => this.refresh([feed])); }))
            .addButton(button => button.setButtonText('Unsubscribe').setDisabled(locked).onClick(() => this.confirm(`Unsubscribe from ${feed.title}?`, 'Remove this source from every folder and delete its local article cache. Reading history and saved notes are kept.', () => this.service.unsubscribe(feed.id))));
        }
      };
    ...
    this.contentEl.createEl('h3', { text: 'Folders (one level)' });
    let title = '';
    new Setting(this.contentEl).setName('New folder')
      .addText(text => text.setPlaceholder('Folder title').onChange(value => { title = value; }))
      .addButton(button => button.setButtonText('Create').setDisabled(locked).onClick(() => { void this.run(() => this.service.createFolder(title)); }));
    for (const folder of snapshot.document.folders) {
      ...
      new Setting(this.contentEl).setName(folder.title)
        ...
        .addButton(button => button.setButtonText('Rename').setDisabled(locked).onClick(() => { void this.run(() => this.service.renameFolder(folder.id, nextTitle)); }))
  ...
  private confirm(title: string, description: string, action: () => Promise<unknown>): void {
    ...
    new Setting(this.contentEl).addButton(button => button.setButtonText('Confirm').setWarning().onClick(() => { void this.run(action); }));
  }
  private transfer(): void {
    ...
    new Setting(this.contentEl).addButton(button => button.setButtonText('Import').setCta().setDisabled(!this.service.getSnapshot().writable).onClick(() => {
      ...
      if (mode === 'replace') this.confirm('Replace all subscriptions?', 'Sources missing from this document will be unsubscribed and their local article cache removed. Reading history and saved notes remain.', () => this.service.import(contents, format, 'replace'));
